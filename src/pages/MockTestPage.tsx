...
export default function MockTestPage() {
  ...
  const { cert, certId, domains, examConfig, activeQuestions } = useCertification();
  const [testStarted, setTestStarted] = useState(false);
  const [result, setResult] = useState<QuizResult | null>(null);
  const [questions] = useState(() => getMockTestQuestions(activeQuestions, examConfig));
  const lowPool = activeQuestions.length < examConfig.totalQuestions;

  function handleComplete(answers: Record<string, number[]>, timeTaken: number) {
    const quizResult = calculateResult(questions, answers, 'mock', Date.now() - timeTaken, undefined, certId);
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Simulate the real {cert.code} exam</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <p className="font-bold text-aws-squid-ink text-lg">{examConfig.totalQuestions}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <span className="text-gray-500">Time Limit</span>
            <p className="font-bold text-aws-squid-ink text-lg">{examConfig.timeLimit / 60000} min</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <span className="text-gray-500">Passing Score</span>
            <p className="font-bold text-aws-squid-ink text-lg">{examConfig.passingScore}%</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
          ...
          {domains.map((d) => (
            <div key={d.id} className="flex items-center justify-between text-sm">
              ...
              <span className="text-gray-500 font-medium">
                {examConfig.domainWeights[d.id]} Qs ({d.percentage}%)
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-amber-800">
          ...
          <ul className="list-disc list-inside space-y-1 text-xs">
            <li>The timer starts immediately — you have {examConfig.timeLimit / 60000} minutes</li>
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-red-800">
            ...
            <p className="text-xs mt-1">
              ...
              exam needs {examConfig.totalQuestions}. Some domains may have fewer
