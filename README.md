...
2. Create and activate a virtual environment (recommended `.venv`; `.env` also works). `.env` conflicts with the common dotenv file naming convention, so prefer `.venv` when possible. Then install dependencies.
   ```bash
   python -m venv .venv
